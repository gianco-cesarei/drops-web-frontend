...
describe('autenticazione App', () => {
  ...
  it('espone navigazione privata approvata senza History o Graph', async () => {
    ...
    expect(links.map((link) => link.getAttribute('href'))).toEqual(expect.arrayContaining(['/', '/app/academy', '/app/content', '/app/download', '/app/archive', '/app/spotify', '/app/radar', '/app/brain', '/app/developer']))
    expect(screen.getByRole('link', { name: 'Impostazioni profilo' })).toHaveAttribute('href', '/app/settings')
    expect(within(nav).getByText(/Admin/i)).toBeInTheDocument()
