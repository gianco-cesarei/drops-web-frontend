    ...
    return content.split('\n').map((line, idx) => {
      ...
      while ((cMatch = comboRegex.exec(line)) !== null) {
        ...
        if (cMatch[1] && cMatch[2]) {
          ...
          // Direct listening guarantee:
          // Whitelist known verified single-track URLs (the 3 radar tracks or direct YouTube watch videos).
          // For any other link (search pages, unverified Bandcamp/SoundCloud links generated by AI),
          // rewrite to the direct listen resolver so the user lands straight on the playing track,
          // without 404s and without having to search or select from lists.
          const VERIFIED_TRACK_URLS = new Set([
            'https://pleasureclubx.bandcamp.com/track/strange-fantasy',
            'https://iliantape.bandcamp.com/track/routine',
            'https://soundcloud.com/max-wiebenga/so-inagawa-logo-queen',
          ])
          ...
          try {
            ...
            const isVerifiedUrl = VERIFIED_TRACK_URLS.has(href)

            if (!isDirectResolver && !isDirectYtWatch && !isVerifiedUrl) {
              // Extract query from URL if present, or extract from line text
              let query = urlObj.searchParams.get('q') || urlObj.searchParams.get('search_query') || ''
              if (!query) {
                // Infer artist & track from current line (e.g. "**Artist** — *Title*" or "- Artist - Title")
                const lineClean = line
                  .replace(/\[([^\]]+)\]\([^)]+\)/g, '') // remove markdown links
                  .replace(/[*_#\[\]]/g, ' ') // remove markdown markers
                  .replace(/\s+/g, ' ')
                  .trim()
                // If linkText itself has something meaningful (e.g. "Ascolta Artist - Track")
                const textClean = linkText.replace(/^(Ascolta su Bandcamp|Ascolta su SoundCloud|Ascolta la Traccia|Ascolta)\s*:?\s*/i, '').trim()
                query = textClean.length > 3 ? textClean : lineClean.slice(0, 80)
              }
              if (query) {
                ...
                href = `/api/v1/curator/listen?q=${encodeURIComponent(query)}&platform=${platform}`
              }
            }
          } catch {}
